...
for imgCount=1:length(direct)
    ...
    if direct(imgCount).bytes < 10000   % Filter based on filesize
      ...
      try  %the try and catch commands will skip contraction videos that trigger errors and keep processing subsequent files in the folder
         ...
         std_Relaxation_Acceleration_time,std_Contraction_time,std_Contraction_Deceleration_time,std_Relaxation_time,std_event_time,std_max_Contract_Vel,std_max_Relax_Vel] = ContractQuant_JKM(pathname,File,dirOut,Tif,offsetMargin);
