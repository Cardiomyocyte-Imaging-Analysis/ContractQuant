function [TrackingDistance_um,PixelDistance,ContractionFrequency,median_max_FrShort,median_min_Length,median_Contract_20Vel,median_Contract_50Vel,median_Contract_80Vel,median_Relax_80Vel,median_Relax_50Vel,median_Relax_20Vel,...
    ...
    std_Relaxation_Acceleration_time,std_Contraction_time,std_Contraction_Deceleration_time,std_Relaxation_time,std_event_time,std_max_Contract_Vel,std_max_Relax_Vel] = ContractQuantM2DManual(offsetMargin)
...
%
% Inputs:
% The ContractQuantManual code is for one-at-a-time analysis of 2DMBs 
% with manual ROI selection. For batch processing with defined ROI locations 
% use ContractQuantBatch instead. For disorganized (nonpatterned) iPSC-CMs,
% use ContractQuantSimple instead. The only input is offsetMargin, for use
% when individual contraction duration is atypical and/or irregular - see
% readme for details.
%
% Outputs:
...
% GUI: Use mouse to get Left and Right tracking points
    fprintf('Please select left and right edges of the tissue.\nPress Enter when you finish.\n')
    [x,y]=ginput;
    x=round(x);
    y=round(y);
    ...
    for i = 1:size(x,1)
        if i == 1
            ROI{i} = Z(y(i)-ROIsize:y(i)+ROIsize,x(i)-ROIsize:x(i)+ROIsize,:);
        else
            ROI{i} = Z(y(i)-ROIsize:y(i)+ROIsize,x(i)-ROIsize:x(i)+ROIsize,:);
        end
    end
